...
let getCategory = async (name, marca) => {
    const options = {
        method: 'GET',
        headers: {
    ...
    try {
        const response = await fetch(`./models/categories/${name}.json`, options);
        const json = await response.json();
        return json
    } catch (err) {
        alert('Error cargando categoria, recargue la página.');
    }
};
...
let BrandCategory = {
    render: async () => {
        ...
        let category = await getCategory(request.name);
        return `
            <div class="row">
                <div class="col p-0">
